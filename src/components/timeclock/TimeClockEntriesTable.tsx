        ...
        {entries.slice(0, 15).map((entry) => {
          ...
          return (
            <div
              key={entry.id}
              className={
                "rounded-2xl border border-border/60 bg-background/70 p-4 shadow-sm " +
                (isEdited(entry) ? "border-destructive/40 bg-destructive/5" : "")
              }
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="font-semibold">{entry.employee ? employeeName : "-"}</p>
                    {isEdited(entry) && (
                      <Badge
                        variant="outline"
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                  ...
                  {entry.clock_out_time
                    ...
                    : renderElapsedTime(entry.clock_in_time)}
                </div>
